...
export default () => {
  ...
  const url = `https://statsapi.web.nhl.com/api/v1/standings/wildCardWithLeaders?expand=standings.team&season=${season}`
  ...
  const buildPlayOff = json => {
    ...
    for (var i = 0; i < json.length; i++) {
      if (json[i].standingsType === 'wildCard') {
        retVal.wildCard[json[i].conference.name] = json[i].teamRecords.slice(
          0,
          2
        )
      } else {
